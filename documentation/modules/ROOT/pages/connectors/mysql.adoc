...
[source,SQL]
First check the warnings if any by running the below command. If warnings are not generated then go ahead.
----
...
. Enable `gtid_mode`:
Follow below steps consecutively to enable gtid_mode.
+
[source,SQL]
----
